...
const int RightMotorSpeed = 16;
const int LeftMotorSpeed = 17; 
...
//update the distance value and check if there is an obstacle 
   Input = US_sensor();
   distance = Input;
   if(Input > 8 && Input < 40){
   myPID.Compute();
  
   DistanceReporting(distance);

   iteration ++;
   analogWrite(LeftMotorSpeed, Output);
   analogWrite(RightMotorSpeed, Output);
   US_Close = false;
   }else{
    US_Close = true;
   }
  ...
  if( digitalRead( LEYE ) == LOW && !US_Close){ // if no black line detected and there is no objects in front of the buggy it will start left motor
    digitalWrite(PosLeftMotor, HIGH); //full power (255)  to motor 
    LeftOn = true; // bool for checking cuurent buggy status in buggy_status();
    }else{
    digitalWrite(PosLeftMotor, LOW);// No power (0) to motor 
  ...
  if( digitalRead( REYE ) == LOW && !US_Close){ // if no black line detected and there is no objects in front of the buggy it will start left motor
    digitalWrite(PosRightMotor, HIGH); //full power (255)  to motor 
    RightOn = true; // bool for checking cuurent buggy status in buggy_status();
  }else{
    digitalWrite(PosRightMotor, LOW); // no power (0) to motor
...
void stopbuggy(){// stops buggy 
  ...
  digitalWrite(PosRightMotor, LOW); // sets the motors value to 0
}
// =================================================================================================================
